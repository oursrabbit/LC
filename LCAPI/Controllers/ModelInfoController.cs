...
namespace LCAPI.Controllers
{
    ...
    public class ModelInfoController : ControllerBase
    {
        ...
        {
            return HttpContext.Request.Headers["LCAPI-MODELINFO"].Equals("Pxq");
        }
        ...
        /// <response code="200">ModelInfo JSON Array</response> 
        ...
        public ActionResult FullTextResourceSearch(FullTextSearchJSON search, int page, int page_size, string order)
        {
            ...
            try
            {
                ...
                var total = 0;
                var res = ModelInfo.FullTextFind(search.searchtext, page, page_size, OrderType, out total);

                var resJson = res.ConvertAll<ModelInfoJSON>(t => t.ToModelInfoJSON());
                ...
                json.Add("models", JArray.Parse(jsonString));
    ...
    $"page: {page}, page_size: {page_size}, order: {order}\r\n\r\n";
            ...
            try
            {
                ...
                var total = 0;
                var res = ModelInfo.Search(search, page, page_size, OrderType, out total);
        ...
        /// 根据ID获取ModelInfoJSON
        /// </summary>
        /// <param name="id">modelinfo.id</param>
        /// <response code="200">ModelInfoJSON</response> 
        /// <response code="403">LCAPI-MODELINFO错误</response>
        ...
        /// </summary>
        /// <param name="id">modelinfo.id</param>
        /// <param name="score">modelinfo.resource_score</param>
        /// <response code="200">返回更新后的modelInfo</response> 
        /// <response code="403">LCAPI-MODELINFO错误</response>
        /// <response code="404">没有找到，id为空</response> 
        /// <response code="500">其他未知错误</response>
        ...
        /// 所有JSON参数都需要填写
        /// </summary>
        /// <param name="modelJson">modelinfo结构，所有属性都需要填写，删除重建，id不变</param>
        /// <response code="200">返回更新后的modelInfo</response> 
        /// <response code="403">LCAPI-MODELINFO错误</response>
        /// <response code="404">没有找到，id为空</response> 
        /// <response code="500">其他未知错误</response>
        ...
        /// <response code="200">Excel文件打开成功，返回三组数据：上传成功的ModelInfoList，ErrorRowIndexList，ErrorString</response> 
        ...
        public async Task<IActionResult> UpdateModelDBFromExcel(IFormFile file, string userId, bool ignore_cell_error)
        {
            ...
            try
            {
                ...
                var newModelList = new List<ModelInfo>();
                var errorRowIndexList = new List<int>();
                ModelInfo.UpdateDBFromExcel(filePath, user, ignore_cell_error, out updateErrorString, out newModelList, out errorRowIndexList);
                errorString += updateErrorString;

                var newModelListJson = newModelList.ConvertAll<ModelInfoJSON>(t => t.ToModelInfoJSON());
                var newModelListJsonString = Newtonsoft.Json.JsonConvert.SerializeObject(newModelListJson);
                ...
                json.Add("new_model", JArray.Parse(newModelListJsonString));
                var returnJson = Newtonsoft.Json.JsonConvert.SerializeObject(json);
        ...
        public IActionResult GetExcelTemplate()
        {
            ...
            try
            {
                ...
                string filePath = $"lc_resource/model_data.xlsx";
                byte[] fileBytes = System.IO.File.ReadAllBytes(filePath);
                return File(fileBytes, "application/octet-stream", "model_data.xlsx");
            }
            catch (Exception ex)
            {
                loggerString += $"error: 500 {ex.Message}";
        ...
        /// 获取数据Excel
        /// </summary>
        /// <param name="ids">modelinfo.id</param>
        /// <response code="200">Excel文件</response> 
        /// <response code="403">LCAPI-MODELINFO错误</response>
        /// <response code="500">其他未知错误</response>
        ...
        public IActionResult GetExcelByIds(List<String> ids)
        {
            ...
            try
            {
                ...
                var models = new List<ModelInfoJSON>();
                ids.ForEach(t => {
                    var m = ModelInfo.GetModelInfoById(t);
                    if (m != null)
                    {
                        models.Add(new ModelInfoJSON(m));
                    }
                });
                ...
                using (ExcelPackage package = new ExcelPackage())
                {
                    ...
                    var colIndex = 1;
                    typeof(ModelInfoJSON).GetProperties().ToList().ForEach(t =>
                    {
                        worksheet.Cells[1, colIndex].SetCellValue(0, 0, t.Name);
                        for (var i = 0; i < models.Count; i++)
                        {
                            var jsonString = Newtonsoft.Json.JsonConvert.SerializeObject(t.GetValue(models[i]));
        ...
        /// <summary>
        /// 总会成功插入数据，错误的属性值会被默认值代替。ModelInfoJSON.id == ""时，插入数据，否则根据id覆盖更新，若未找到则直接插入
        /// </summary>
        /// <param name="model">新的数据</param>
        /// <response code="200">插入成功，并返回带有id的ModelInfoJSON</response> 
        /// <response code="403">LCAPI-MODELINFO错误</response>
        /// <response code="500">其他未知错误</response>
        ...
        public ActionResult InsertModelInfo(ModelInfoJSON modelJson)
        {
            ...
            try
            {
                ...
                var newModel = new ModelInfo(modelJson);
                var insertModel = ModelInfo.Insert(newModel);
                insertModel = insertModel ?? new ModelInfo();
