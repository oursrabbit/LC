...
namespace LCAPI.Controllers
{
    ...
    public class DocInfoController : ControllerBase
    {
        ...
        public DocInfoController(ILogger<DocInfoController> _)
        {
        ...
        {
            return HttpContext.Request.Headers["LCAPI-DOCINFO"].Equals("PXq");
        }

        /// <summary>
        /// 对文档资源信息进行全文查找
        /// 
        /// 查找的范围参看FullText结构，目前包括范围：描述，文件名，关键词，领域
        /// </summary>
        ...
        /// <response code="200">DocInfo JSON Array</response> 
        /// <response code="403">LCAPI-DOCINFO错误</response>
        ...
        public ActionResult FullTextResourceSearch(FullTextSearchJSON search, int page, int page_size, string order)
        {
            try
            {
                ...
                var total = 0;
                var res = DocInfo.FullTextFind(search.searchtext, page, page_size, OrderType, out total);

                var resJson = res.ConvertAll<DocInfoJSON>(t => t.ToDocInfoJSON());
                ...
                json.Add("docs", JArray.Parse(jsonString));
        ...
        [Produces("application/json")]
        public ActionResult PropertySearch(SearchDocJSON search, int page, int page_size, string order)
        {
            try
            {
                loggerString = $"searchJSON: {Newtonsoft.Json.JsonConvert.SerializeObject(search)}\r\n\r\n" +
$"page: {page}, page_size: {page_size}, order: {order}\r\n\r\n";
                ...
                var total = 0;
                var res = DocInfo.Search(search, page, page_size, OrderType, out total);
        ...
        /// 根据ID获取DocInfoJSON
        /// </summary>
        /// <param name="id">docinfo.id</param>
        /// <response code="200">DocInfoJSON</response> 
        /// <response code="403">LCAPI-DOCINFO错误</response>
        /// <response code="404">没有找到，id为空</response> 
        /// <response code="500">其他未知错误</response>
        [HttpGet("Search/Doc")]
        [Produces("application/json")]
        public ActionResult GetDocInfoById(string id)
        {
            try
            {
                loggerString = $"id: {id}\r\n\r\n";
                ...
                var doc = DocInfo.GetDocInfoById(id);

                if (doc == null)
                {
                    loggerString += $"error: 403";
                    return RestResultJSON.CreateRestJSONResult("404", "未找到", new DocInfoJSON(), 404);
                }
                else
                {
                    var json = Newtonsoft.Json.JsonConvert.SerializeObject(new DocInfoJSON(doc));
        ...
        /// </summary>
        /// <param name="id">docinfo.id</param>
        /// <param name="score">docinfo.resource_score</param>
        /// <response code="200">返回更新后的docInfo</response> 
        /// <response code="403">LCAPI-DOCINFO错误</response>
        /// <response code="404">没有找到，id为空</response> 
        /// <response code="500">其他未知错误</response>
        ...
        public ActionResult UpdateScoreByID(string id, double score)
        {
            try
            {
                ...
                var doc = DocInfo.UpdateScore(id, score);
        ...
        public ActionResult UpdateDownloadCountByID(string id)
        {
            try
            {
                ...
                var doc = DocInfo.UpdateDownloadCount(id);
        ...
        public ActionResult UpdateViewCountByID(string id)
        {
            try
            {
                ...
                var doc = DocInfo.UpdateViewCount(id);
        ...
        /// 所有JSON参数都需要填写
        /// </summary>
        /// <param name="docJson">docinfo结构，所有属性都需要填写，删除重建，id不变</param>
        /// <response code="200">返回更新后的docInfo</response> 
        /// <response code="403">LCAPI-DOCINFO错误</response>
        /// <response code="404">没有找到，id为空</response> 
        /// <response code="500">其他未知错误</response>
        [HttpPost("Update/Doc")]
        public ActionResult UpdateDocInfo(DocInfoJSON docJson)
        {
            loggerString = $"\r\n\r\ncall: UpdateDocInfoByID\r\n\r\n" + $"docJson_new: {Newtonsoft.Json.JsonConvert.SerializeObject(docJson)}\r\n\r\n";
            Request.Headers.ToList().ForEach(t => loggerString += $"http-header: {t.Key} http-value: {t.Value}\r\n\r\n");
            ...
            try
            {
                ...
                var doc = DocInfo.Update(new DocInfo(docJson));
        ...
        /// <response code="200">Excel文件打开成功，返回三组数据：上传成功的DocInfoList，ErrorRowIndexList，ErrorString</response> 
        /// <response code="422">Excel文件处理失败</response>
        /// <response code="403">LCAPI-DOCINFO错误</response>
        /// <response code="404">ignore_cell_error = true时，userid未找到</response>
        /// <response code="500">其他未知错误</response>
        [HttpPost("Update/Excel")]
        public async Task<IActionResult> UpdateDocDBFromExcel(IFormFile file, string userId, bool ignore_cell_error)
        {
            ...
            try
            {
                ...
                var newDocList = new List<DocInfo>();
                var errorRowIndexList = new List<int>();
                DocInfo.UpdateDBFromExcel(filePath, user, ignore_cell_error, out updateErrorString, out newDocList, out errorRowIndexList);
                errorString += updateErrorString;

                var newDocListJson = newDocList.ConvertAll<DocInfoJSON>(t => t.ToDocInfoJSON());
                var newDocListJsonString = Newtonsoft.Json.JsonConvert.SerializeObject(newDocListJson);
                ...
                json.Add("new_doc", JArray.Parse(newDocListJsonString));
                var returnJson = Newtonsoft.Json.JsonConvert.SerializeObject(json);
        ...
        /// <response code="500">其他未知错误</response>
        [HttpGet("Delete/Doc")]
        public ActionResult DeleteDocInfoByID(string id)
        {
            var newGuid = Guid.NewGuid();
            loggerString = $"\r\n\r\ncall: DeleteDocInfoByID\r\n\r\n" + $"id: {id}\r\n\r\n";
            Request.Headers.ToList().ForEach(t => loggerString += $"http-header: {t.Key} http-value: {t.Value}\r\n\r\n");
        ...
        public IActionResult GetExcelTemplate()
        {
            ...
            try
            {
                ...
                string filePath = $"lc_resource/doc_data.xlsx";
                byte[] fileBytes = System.IO.File.ReadAllBytes(filePath);
                return File(fileBytes, "application/octet-stream", "doc_data.xlsx");
            }
            catch (Exception ex)
            {
                loggerString += $"error: 500 {ex.Message}";
        ...
        /// 获取数据Excel
        /// </summary>
        /// <param name="ids">docinfo.id</param>
        /// <response code="200">Excel文件</response> 
        /// <response code="403">LCAPI-DOCINFO错误</response>
        /// <response code="500">其他未知错误</response>
        ...
        public IActionResult GetExcelByIds(List<String> ids)
        {
            ...
            try
            {
                ...
                var docs = new List<DocInfoJSON>();
                ids.ForEach(t => {
                    var m = DocInfo.GetDocInfoById(t);
                    if(m != null)
                    {
                        docs.Add(new DocInfoJSON(m));
                    }
                });
                    
                using (ExcelPackage package = new ExcelPackage())
                {
                    ExcelWorksheet worksheet = package.Workbook.Worksheets.Add("Doc");
                    var colIndex = 1;
                    typeof(DocInfoJSON).GetProperties().ToList().ForEach(t =>
                    {
                        worksheet.Cells[1, colIndex].SetCellValue(0, 0, t.Name);
                        for (var i = 0; i < docs.Count; i++)
                        {
                            var jsonString = Newtonsoft.Json.JsonConvert.SerializeObject(t.GetValue(docs[i]));
        ...
        /// <summary>
        /// 总会成功插入数据，错误的属性值会被默认值代替。DocInfoJSON.id == ""时，插入数据，否则根据id覆盖更新，若未找到则直接插入
        /// </summary>
        /// <param name="doc">新的数据</param>
        /// <response code="200">插入成功，并返回带有id的DocInfoJSON</response> 
        /// <response code="403">LCAPI-DOCINFO错误</response>
        /// <response code="500">其他未知错误</response>
        [HttpPost("Insert/DocInfo")]
        public ActionResult InsertDocInfo (DocInfoJSON docJson)
        {
            loggerString = $"\r\n\r\ncall: InsertDocInfo\r\n\r\n" + $"userInfoJSON: {Newtonsoft.Json.JsonConvert.SerializeObject(docJson)}\r\n\r\n";
            Request.Headers.ToList().ForEach(t => loggerString += $"http-header: {t.Key} http-value: {t.Value}\r\n\r\n");
            ...
            try
            {
                ...
                var newDoc = new DocInfo(docJson);
                var insertDoc = DocInfo.Insert(newDoc);
                insertDoc = insertDoc ?? new DocInfo();

                var json = Newtonsoft.Json.JsonConvert.SerializeObject(new DocInfoJSON(insertDoc));
